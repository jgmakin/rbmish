function [Zout,Uout,LDSparams] = getCenterOutReacher(yrclass,Ntraj,T,...
    dynamics,Ndims,mods,smin,smax)
...
% useful params
HHSdt = 1/240;                              % fact
NsamplesPerBin = dynamics.dt/HHSdt;  % convert to longer bins
...
[Zout,Uout,LDSparams.G] = getCenterOutControls(Z,LDSparams,smin,smax,mods);
...
if 1
    ...
    cntr = LDSparams.cntr;
    x = X(:,1:Ndims);
    cntrDist = squeeze(sqrt(sum((x - shiftdim(cntr,-1)).^2,2)));
    v = X(:,(Ndims+1):2*Ndims);
...
while NEEDMOREBATCHES
    ...
    thisOutCenter = (R*(hisCenterOut - xF)')' + cntr;
...
vecsToTargs = Zfinal(:,1:Ndims) - shiftdim(targets',-1);
...
% wrap the data
Xscaled = shortdata(Ntrials,3,scalefxn(longdata(X),xmin,xmax,...
    zeros(2,1),2*pi*ones(2,1)));
