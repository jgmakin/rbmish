...
% USAGE:
%
%   load([getdir('data'),'\RBMish\EFHs\wts_rEFH_1D_LTI-PPC_NoSpring_ManySpeeds.mat'])
%   i = 3; 
...
T = Qtest.T;
Ntraj = floor(size(Rtest,1)/T);
[~,V0] = updownRDBN(Rtest,wts,params,T);
V0 = shortdata(Ntraj,3,V0);
...
switch params.datatype
    case 'HHSreachData'
        ...
        mvmtAng = shortdata(Ntraj,2,atan2(v(:,2),v(:,1)));
    ...
    otherwise
        ...
        Th = squeeze(shortdata(Ntraj,4,S(:,:,strcmp(params.mods,params.NS))));
        %%%[taus,maxMI,Ptau,Pmax] = getOptimalLag(V0,Th,-40,20,params);
        [taus,maxMI,Ptau,Pmax] = getOptimalLag(V0,Th,-40,0,params);
end
...
Thdot = squeeze(shortdata(Ntraj,3,Xtest(:,2)));
Nthbins = 30;
Nthdotbins = 30;
...
X0 = getIdealResponses(ThIncidence,Ptau,taus,T);
...
normalizedAC = (truncatedAC./acArea)*histArea;
...
%-------------------------------------------------------------------------%
function X0 = getIdealResponses(ThIncidence,Ptau,taus,T)

Nneurons = size(Ptau,1);
Ntraj = size(ThIncidence,1)/(T-1);

PrXis1 = reshape(ThIncidence*Ptau',[Ntraj,T-1,Nneurons]);
X0 = zeros(Ntraj,Nneurons,T);
