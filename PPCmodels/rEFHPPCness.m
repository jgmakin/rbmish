...
%%%%%%%
% This function needs to be repaired (numsUnits, typeUnits, etc.)
%   (02/25/16 - jgm)
%%%%%%% 
...
[ZbarTest,Ztest,Stest,pEMtest,pMIRRORtest,pREFHtest] =...
    getXY(T,LDSparamsEM,LDSparamsREFH,wts,params,20);
[ZbarTrain,Ztrain,Strain,pEMtrain,pMIRRORtrain,pREFHtrain] =...
    getXY(T,LDSparamsEM,LDSparamsREFH,wts,params,40);
...
function [Zbar,Z,S,pEM,pMIRROR,pREFH] =...
    getXY(T,LDSparamsEM,LDSparamsREFH,wts,params,Ntraj)
...
% filter with rEFH
[R1,Zbar] = updownRDBN(R0,wts,params,T);
...
Z = sampleT(Zbar,params.typeUnits{2},params.numsUnit{2},params);

% filter with other models
pMIRROR = KFposteriorization(pSENSORY,Q,LDSparamsREFH,params); 
pMIRROR.name = 'mirror';
pEM = KFposteriorization(pSENSORY,Q,LDSparamsEM,params); 
pEM.name = 'EM2';
