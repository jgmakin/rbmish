...
% [D0 x0 G0] = generateData(1000,params);
%-------------------------------------------------------------------------%
% Revised: 10/01/12
...
% malloc if necessary
if isempty(xx)
    xx = zeros(Nexamples,params.Ndims*length(params.mods));
    DECODE = 1;
else
    if ~isempty(dd)
...
for i = 1:Nbatches
    ...
    hidmeans = invParamMap(D0(:,:,i),wts{1}(1:end-1,:),wts{1}(end,:),...
        hidDstrbs,hidNums,params);
    
    NNNN = params.smpls;
    hidstates = sampleT(hidmeans,hidDstrbs,hidNums,params);      % samples!
    for j=1:NNNN-1
        hidstates = hidstates + sampleT(hidmeans,hidDstrbs,hidNums,params);
    end
    numspikes(i) = mean(sum(hidstates,2));
    % numspikes = mean(mean(sum(V0,2),3));
    
    hidstates = hidstates/NNNN;
    % V0(:,:,i) = states;
    
    D1(:,:,i) = invParamMap(hidstates,wts{2}(1:end-1,:),wts{2}(end,:),...
        visDstrbs,visNums,params);
    D2(:,:,i) = invParamMap(hidmeans,wts{2}(1:end-1,:),wts{2}(end,:),...
        visDstrbs,visNums,params);
