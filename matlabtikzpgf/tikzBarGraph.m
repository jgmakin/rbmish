...
[defaultExtraCode,defaultExtraAxisOptions] = wrapperbits(title);

% adjust the style definition for y tick labels to set the precision
i_yticklabelHider = contains(defaultExtraAxisOptions,'pacity=\thisYticklabelopacity');
yticklabelHider = defaultExtraAxisOptions{i_yticklabelHider}(1:end-1);
defaultExtraAxisOptions{i_yticklabelHider} = sprintf([...
    '%s\n\t',...
    '/pgf/number format/fixed,%%\n\t',...
    '/pgf/number format/precision=5,%%\n',...
    '}'],yticklabelHider(1:end-1));
%%% evidently you don't want this done in matlab2tikzWrapper, which also
%%% calls wrapperbits.m, otherwise you'd place it in there.  Check me?
...
% text to be written
defaultExtraCode = arrayfun(@(ii)([defaultExtraCode{ii},newline]),...
    1:length(defaultExtraCode),'UniformOutput',false);
outtxt = [defaultExtraCode{:},...
...
defaultExtraAxisOptions = arrayfun(@(ii)([defaultExtraAxisOptions{ii},...
    ',',newline]),1:length(defaultExtraAxisOptions),'UniformOutput',false);
outtxt = [outtxt,defaultExtraAxisOptions{:}];
