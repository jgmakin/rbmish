...
tikzBarGraph(avgKLs,avgKLerrorBars,2,0.32,xaxislabels,[],'nats',titlestr,...
...
tikzBarGraph(avgKLsbits,avgKLerrorBars,2,0.62,xaxislabels,[],'bits',titlestr,...
    {'rbmclr','rbmclr','optclr'},['posteriorKLdivsbits',date]);
...
tikzBarGraph([Rsqq Rsqr],RSQerrorBars,2,0.32,xaxislabels,[],'$R^2$',...
    titlestr,{'rbmclr','rbmclr','rbmclr','rbmclr'},['etarecovery',date]);
...
% fit an affine ("linear") model
[beta1, RsqCV1] = linrgsLOOCV(X1,Y1);
[beta2, RsqCV2] = linrgsLOOCV(X2,Y2);
...
InfoINT = sum(bsxfun(@times,p.Info(testvec,:,:,1:2),...
