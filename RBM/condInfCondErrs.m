...
%%%%%% TO DO %%%%%%
% (1) fix (esp. plotting?) to work with params.Nmods = 3, so that you can check
...
numlocs = size(proplocs,2);                     % how many locs do we test?
Nbatches = floor(Nexamples/params.Ncases);      % make sure it's an integer
Nmods = params.Nmods;
...
for loc = 1:numlocs
    ...
    p0.mu = scalefxn(proplocs(:,loc),[0;0],[1;1],params.thmin,params.thmax);
    p0.cov = 0;
    
    % generate data at one point
    [D0,S0] = DATAGENPP(Nbatches,params,'prior',p0);
...
%         [Di S.zvec] = killoneinput(D0,params.mods{modeOFF},params.Nmods);
%         
...
switch params.Ndims
    case 1  % 1D vars
        ...
        % outputvar = statsFinal{params.Nmods + 2}.cov;
