function v = vissampler(M,params,varargin)
...
x = scalefxn(rand(2,M),[0;0],[1;1],xmin(:,iSquare),xmax(:,iSquare));
...
for i = 1:size(v,2)
    inds(i) = ~outofrange(v(:,i),posmin,posmax,'Hand-Position',0,params);
end
v = v(:,logical(inds));
P = M - size(v,2);
...
if P > 0
    ...
    v1 = vissampler(P,params,VERBOSE);
    v = [v v1];
else
    if VERBOSE; fprintf('\n'); end
end
