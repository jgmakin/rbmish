function y = poisslpdf(x,lambda)
%POISSPDF Poisson *log* probability density function.
%   Y = POISSLPDF(X,LAMBDA) returns the Poisson log probability density 
...
%-------------------------------------------------------------------------%
% Revised: 09/06/16
%   -For use in calculations that avoid exponentiating log probabilities!
%   The only actual change is the elimination of exp{} in three places.
%   -cut and pasted some functions into this one (they were hidden inside
%   the statistics toolbox):
%       --stirlerr.m
%       --binodeviance.m
% Cribbed: 09/06/16
%   from the Mathworks's poisspdf.m
%   by JGM
%-------------------------------------------------------------------------%
...
bd0=zeros(size(x),'like',internal.stats.dominantType(x,np));
