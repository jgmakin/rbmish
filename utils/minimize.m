...
% derivatives of f wrt X, the "length" gives the length of the run: if it
% is positive, it gives the maximum number of line searches, if negative
% its absolute gives the maximum allowed number of function evaluations.
% You can (optionally) give "length" a second component, which will
% indicate the reduction in function value to be expected in the first
% line-search (defaults to 1.0). The parameters P1, P2, P3, ... are passed
% on to the function f.
%
...
% due to numerical problems, we cannot get any closer). NOTE: If the
% function terminates within a few iterations, it could be an indication
% that the function values and derivatives are not consistent (ie, there
% may be a bug in the implementation of your "f" function). The function
% returns the found solution "X", a vector of function values "fX"
...
% Wolfe-Powell conditions. SIG is the maximum allowed absolute ratio
% between previous and new slopes (derivatives in the search direction),
% thus setting SIG to low (positive) values forces higher precision in the
% line-searches. RHO is the minimum allowed fraction of the expected (from
% the slope at the initial point in the linesearch). Constants must satisfy
% 0 < RHO < SIG < 1. Tuning of SIG (depending on the nature of the function
% to be optimized) may speed up the minimization; it is probably not worth
...
% acceptable point is found (Wolfe-Powell conditions). Note, that points
% are always maintained in order p0 <= p1 <= p2 < p3 < p4. 3) compute a new
