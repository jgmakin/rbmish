...
%-------------------------------------------------------------------------%
% Cribbed: 06/24/16
%   from eegfilt.m, from the eeglab toolbox.  Edited to use matrix ops
%   JGM
%-------------------------------------------------------------------------%

function [smoothdata,filtwts] = eegfiltJGM(data,srate,locutoff,hicutoff,epochframes,filtorder,revfilt,firtype,causal)
...
for e = 1:epochs                % filter each epoch, channel
    ...
    %%% You've replaced the loop over filters with provision of the entire
    %%% matrix, data, to filter or filtfilt.  UNFORTUNATELY, eegfilt was
    %%% written with channels in rows, rather than columns (as expected by
    %%% filter or filtfilt), hence the pair of transposes in each line
    %%% below.  The *second* (outer) transpose respects the original design
    %%% of eegfilt; i.e., it still returns data in Nchannels x Nsamples
    %%% format.
    %%%
    %%% JGM
    try
        if causal
            smoothdata(:,(e-1)*epochframes+1:e*epochframes) =...
                filter(filtwts,1,data(:,(e-1)*epochframes+1:e*epochframes)')';
        else
            smoothdata(:,(e-1)*epochframes+1:e*epochframes) =...
                filtfilt(filtwts,1,data(:,(e-1)*epochframes+1:e*epochframes)')';
        end;
    catch ME
        if causal
            smoothdata(:,(e-1)*epochframes+1:e*epochframes) =...
                filter(filtwts,1,double(data(:,(e-1)*epochframes+1:e*epochframes))')';
        else
            smoothdata(:,(e-1)*epochframes+1:e*epochframes) =...
                filtfilt(filtwts,1,double(data(:,(e-1)*epochframes+1:e*epochframes))')';
        end;
    end;
end
fprintf('\n');
